...
class GF_User_Registration extends GFFeedAddOn {
	...
	public function create_user( $entry, $form, $feed = false, $password = '', $password_hash = '' ) {
		...
		wp_update_user( array( 'ID' => $user_id, 'display_name' => $this->get_display_name( $user_id, $feed ) ) );

		// This should always run after the user has been updated otherwise the hashed password will be rehashed.
		if( $user_data['password_hash'] ) {
			// Let's overwrite the password with the hashed version
			$this->log( sprintf( 'Setting hashed password for user ID %d.', print_r( $user_id, true ) ) );
	...
	public function get_send_email_type( $feed ) {
		...
		if ( isset( $feed['meta']['sendEmailEnable'] ) ) {
			return $feed['meta']['sendEmailEnable'] ? rgar( $feed['meta'], 'sendEmailValue' ) : false;
		}
		...
		return (bool) $feed['meta']['sendEmail'];
	}
	...
	public function settings_send_email( $field, $echo = true ) {
		...
		$html .= sprintf( '<div id="sendEmailLocked" class="alert_yellow" style="display:none;">%s</div>',
